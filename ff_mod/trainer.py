...
class Trainer:
    def __init__(self, unsupervised = False, device = "cuda:0", greedy_goodness = False, is_conv = False) -> None:
        ...
        self.dynamic_save = False
        self.dynamic_save_path = None
    
    def set_feature_extractors(self, feature_extractors):
        self.feature_extractors = feature_extractors
    
    def set_dynamic_save(self, dynamic_save_path):
        self.dynamic_save = True
    ...
    def train(self, epochs: int = 2, verbose: int = 1):
        for epoch in range(epochs):
            ...
            if self.dynamic_save:
                self.__net.save_network(self.dynamic_save_path)
                
                # Save the epoch to train_info.json
                with open(f"{self.dynamic_save_path}train_info.json", 'w') as file:
                    json.dump({"epoch": epoch}, file, indent=4)