...
def save_auroc_curve(labels, scores, save_path, algorithm_name = "FF_OOD"):
    ...
    plt.savefig(save_path+f"/{algorithm_name}.pdf")
    
def save_score_dist(labels, scores, save_path, algorithm_name = "FF_OOD"):
    
    plt.clf()
    
    plt.hist([scores[i] for i in range(len(scores)) if labels[i] == 0], bins=100, alpha=0.5, label='ID Scores', color='blue', density=True)
    plt.hist([scores[i] for i in range(len(scores)) if labels[i] == 1], bins=100, alpha=0.5, label='OOD Scores', color='red', density=True)
    ...
    plt.savefig(save_path+f"/{algorithm_name}_dist.pdf")
