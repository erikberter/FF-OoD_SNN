...
class IncludeChannelOverlay(Overlay):
    def __init__(self, channel_n: int, value: float):
        self.channel_n = channel_n
        self.value = value
    
    def __call__(self, data, label=None, **kwargs):
        ## Appends channel_n new channels to the end of the image. 
        ## for each sample of the batch, only the channel corresponing to the label is set
        ## to value
        
        

        if len(data.shape) == 4:
            # Image Data [batch, c, h, w]
            IN_CHANNELS = data.shape[1]
            data_ = torch.cat([data, torch.zeros(data.shape[0], self.channel_n, data.shape[2], data.shape[3]).to(data.device)], 1)
            data_[range(data.shape[0]), IN_CHANNELS + label] = self.value
        else:
            # Spiking Data [batch, t, c, h, w]
            IN_CHANNELS = data.shape[2]
            data_ = torch.cat([data, torch.zeros(data.shape[0], data.shape[1], self.channel_n, data.shape[3], data.shape[4]).to(data.device)], 2)
            data_[range(data.shape[0]), :, IN_CHANNELS + label] = self.value
